...
forv h=0/4 {
	...
	*** LPIV
		xi: xtivreg2 S`h'p (SdCAPB`h' = size) _x* if exp==1 , fe cluster(iso)
...
gen FE = I*1000+H // e.g. USA at horizon 1 is 111001 etc. 
	*** LPIV stacked
	tsset FE year
	noi xi: ivreg2 Y (T_* = Z_*) X* i.FE if exp==1 , cluster(FE) // dkraay(3) //*
...
(rcap  U D  h  if h==5, vert lc(green%50))  (scatter B h if h==5 , mlab(B) mlabc(green) mc(green) ) ///
(rarea U D  h  if h<5,  fcolor(green%15) lcolor(gs13) lw(none) lpattern(solid)) ///
(line b h if h<5 , lcolor(green%50) lpattern(solid) lwidth(thick)) ///
...
name(Mboom,replace)
gr export Mexp_RMORT.pdf , replace
